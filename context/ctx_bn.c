...
BOOLEAN BN_MONT_CTX_set( INOUT BN_MONT_CTX *bnMontCTX, const BIGNUM *mod, 
	...
	/* Set up the auxiliary modulus R as a value one larger than a bignum 
	   ...
	   (this fulfils that requirement that R > N, or in this case 
	   R > modWord).  
...
BOOLEAN BN_from_montgomery( INOUT BIGNUM *ret, INOUT BIGNUM *aTmp,
	...
	BN_ULONG *aData = aTmp->d, carry = 0;
	const int nLen = N->top;
	int i;
	...
	/* If the values get very large then we need to use an extended bignum 
	   as a temporary.  The use of the bignum looks a bit odd since the 
	   value is local to this code block, but all we need is the memory 
	   associated with it, and cleanup is done via the BN_CTX_end_ext() */
	...
	BN_normalise( ret );	/* Sanity-checked below */
	if( aData != aTmp->d )
		BN_CTX_end_ext( bnCTX, BIGNUM_EXT_MONT );
