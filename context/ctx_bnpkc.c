...
void endContextBignums( INOUT PKC_INFO *pkcInfo, 
	...
	assert( sanityCheckPKCInfo( pkcInfo ) );
			/* We don't make this an ENSURES since that would prevent 
			   clearing the data, since it's just a series of memset()s
			   of static-sized data it doesn't matter if a field is in 
...
static BN_ULONG MurmurHash3( const BN_ULONG *data, const int len, BN_ULONG seed )
	...
	/* Finalisation and avalanche, fmix32() vesion of the 64-bit fmix64(), 
	   see the comment on that for how it works */
	h1 ^= len;
	h1 ^= h1 >> 16;
	h1 *= 0x85EBCA6BUL;
	h1 ^= h1 >> 13;
...
static BOOLEAN checksumStaticBignum( const BIGNUM *bignum,
	...
	/* Make sure that the metadata fields are valid.  These have fixed 
	   ...
	   supposed to */
	if( bignum->dmax != BIGNUM_ALLOC_WORDS || \
		bignum->neg != FALSE || \
	...
	for( i = bignum->top; i < BIGNUM_ALLOC_WORDS; i++ )
		{
		if( bignum->d[ i ] != 0 )
			return( FALSE );
		}
