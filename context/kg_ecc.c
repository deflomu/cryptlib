...
static int checkECCDomainParameters( INOUT PKC_INFO *pkcInfo,
	...
	int length, i, pLen, tmp3hBits, nBits, bnStatus = BN_STATUS;
	...
	if( !primeSieve( p ) )
		return( CRYPT_ARGERROR_STR1 );
#if 0
	status = primeProbable( pkcInfo, p, 1 );
	if( cryptStatusError( status ) )
		return( status );
	if( status != TRUE )
		return( CRYPT_ARGERROR_STR1 );
	...
	if( !primeSieve( n ) )
		return( CRYPT_ARGERROR_STR1 );
#if 0
	stauts = primeProbable( pkcInfo, n, 1 );
	if( cryptStatusError( status ) )
		return( status );
	if( status != TRUE )
		return( CRYPT_ARGERROR_STR1 );
...
static int checkECCPublicKey( INOUT PKC_INFO *pkcInfo )
	...
	if( !isPointOnCurve( qy, qx, &domainParams->a, &domainParams->b, 
						 pkcInfo ) )
		return( CRYPT_ARGERROR_STR1 );
...
int initCheckECCkey( INOUT CONTEXT_INFO *contextInfoPtr,
	...
	/* Make sure that the necessary key parameters have been initialised.  
	   We know that the domain parameters are initialised since they're 
	   hardcoded in as the ECC_DOMAINPARAMS structure */
	if( !isECDH )
		{
