...
/* Error-detecting function pointers.  We store two copes of the pointer, 
   the value itself and its bitwise inverse.  If on retrieving them their 
   XOR isn't all-ones then one of the values has been corrupted and the 
   pointer isn't safe to dereference.  The macros are used as:

	FNPTR_DECLARE( PTR_TYPE, ptrStorage );

	FNPTR_SET( ptrStorage, functionAddress );
	functionPtr = FNPTR_GET( ptrStorage );
	ENSURES( functionPtr != 0 );

   In terms of what to store and how, would could store two copies of the 
...
#define FNPTR_ISSET( name ) \
		( ( name##1 ) != NULL && ( name##2 ) != 0 )

#define FNPTR_GET( name ) \
		( ( ( ( ( FNPTR_TYPE ) name##1 ) ^ ( name##2 ) ) != ~0 ) ? NULL : name##1 )
