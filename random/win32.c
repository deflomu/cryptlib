...
/*	addRandomValue( randomState, GetQueueStatus( QS_ALLEVENTS ) ); */
	...
	if( getSysVar( SYSVAR_HWCAP ) & HWCAP_FLAG_XSTORE )
		...
		/* VS 2015 will warn (incorrectly) about EBX being trashed, assuming 
		   ...
		   the Centaur 0xC0000001 op only writes to EDX.  There's no easy way 
		   to disable this warning, #pragma warning( disable: 4713 ) only 
		   works on a per-function level which means that it'd need to be 
		   disabled for the entire function just to get rid of the spurious
		   warning when cpuid is called (and a second time for the pop ebx) */
		__asm {
			push es
