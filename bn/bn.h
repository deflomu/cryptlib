...
#define BN_FLG_MAX			0x37	/* Maximum possible flag value */

/* Bignum data types and sizes.  No idea how the BN_BITS is supposed to work, 
   it's only used in one location in bn_gcd.c where it's used as if it 
   indicates the machine word size, technically it's OK for THIRTY_TWO_BIT 
   because it assumes there's a 64-bit BN_ULLONG, but it's definitely wrong 
   for the others.  We keep it as is because the BN_BITSn defines and 
   therefore the OpenSSL code all depend on it */
...
/* 64-bit architectures where long and long long are 64 bits, e.g. x86-64 
   Unix, Alpha, and Sparc64 */

#ifdef SIXTY_FOUR_BIT_LONG
  #define BN_ULLONG			unsigned LONGLONG_TYPE
...
/* 64-bit architectures where long long is 64 bits but long is 32 bits,
  ...
  #define BN_ULONG			unsigned LONGLONG_TYPE
  #define BN_BITS			128
...
/* Support functions to mask the OpenSSL memory-allocation mess */
...
BOOLEAN BN_mod_mul_montgomery( INOUT BIGNUM *r, const BIGNUM *a, 
							   ...
							   INOUT BN_CTX *bnCTX );
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1, 2, 3, 4 ) ) \
BOOLEAN BN_from_montgomery( INOUT BIGNUM *ret, BIGNUM *aTmp,
							INOUT BN_MONT_CTX *bnMontCTX,
							INOUT BN_CTX *bnCTX );
