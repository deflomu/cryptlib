...
if [ `uname -s` = "Linux" ] ; then
	if [ `hostname -f | grep -c login0[0-9]\.fos\.auckland\.ac\.nz` -gt 0 ] ; then
		ISDEVELOPMENT=1 ;
	elif [ `hostname -f | grep -c [a-z]\.cypherpunks\.to` -gt 0 ] ; then
		ISDEVELOPMENT=1 ;
	elif [ `hostname -f` = "ci20.lan" -a `date +%Z` = "NZDT" ] ; then
		ISDEVELOPMENT=1 ;
...
if [ $GCC_VER -ge 30 -a $GCC_VER -le 46 ] ; then
...
if [ $GCC_VER -ge 47 -a $GCC_VER -le 49 ] ; then
