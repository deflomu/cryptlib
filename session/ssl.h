...
/* SSL/TLS protocol-specific flags that augment the general session flags:
	...
	SSL_PFLAG_EMS: Use extended Master Secret to protect handshake messages.
...
#define SSL_PFLAG_MAX				0x07FF	/* Maximum possible flag value */
...
typedef enum {
	...
	TLS_EXT_TEMP_TOKENBIND,		/* 24: Draft, Token binding */
		/* 25...34 unused */
...
/* When building the debug version of the code we include a text string 
   describing the cipher suite that's being processed, this makes it easier 
...
typedef struct SL {
	...
	/* Session hash, the hash of all messages from Client Hello to Client 
	   Keyex */
	...
	FNPTR_DECLARE( SSL_HANDSHAKE_FUNCTION, beginHandshake );
	FNPTR_DECLARE( SSL_HANDSHAKE_FUNCTION, exchangeKeys );
	} SSL_HANDSHAKE_INFO;
