...
int encryptData( const SESSION_INFO *sessionInfoPtr, 
	...
	*dataLength = 0;
	...
	if( sessionInfoPtr->cryptBlocksize > 1 )
		{
#ifdef USE_SSL3
		const int maxPadSize = ( sessionInfoPtr->version == SSL_MINOR_VERSION_SSL ) ? \
								 sessionInfoPtr->cryptBlocksize : MESSAGE_PADDING_SIZE;
		const int padSize = ( maxPadSize - 1 ) - \
						    ( payloadLength & ( maxPadSize - 1 ) );
#else
		const int padSize = ( MESSAGE_PADDING_SIZE - 1 ) - \
						    ( payloadLength & ( MESSAGE_PADDING_SIZE - 1 ) );
#endif /* USE_SSL3 */
		int i;

		ENSURES( padSize >= 0 && padSize < MESSAGE_PADDING_SIZE && \
...
static int getSessionHash( IN_HANDLE const CRYPT_CONTEXT iHashContext,
	...
	assert( isWritePtr( hashValue, hashValueMaxLen ) );
	assert( isWritePtr( hashValueLen, sizeof( int ) ) );

	REQUIRES( isHandleRangeValid( iHashContext ) );
	REQUIRES( hashValueMaxLen >= 16 && \
